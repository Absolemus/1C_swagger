...
Функция Прокси(HTTPЗапрос)  
	...
	СваггерВключен = Константы.СваггерВключен.Получить();
	
	Если (ЭтоЗапросСваггер Или ЭтоЗапросДокументации) И Не СваггерВключен Тогда
		Ответ = Новый HTTPСервисОтвет(403, "Service Not Available");
		Ответ.УстановитьТелоИзСтроки("This service is not available in the production environment.");
		Возврат Ответ;
	ИначеЕсли ЭтоЗапросСваггер Тогда
...
Функция Сваггер(HTTPЗапрос) 	
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	""/"" + ДополнительныеОтчетыИОбработкиКоманды.Префикс + ""/swagger.json"" КАК config_url,
	               ...
	               |	Справочник.ДанныеСервера КАК ДанныеСервера
	               |ГДЕ
	               |	ИСТИНА
...
Функция Сервис(HTTPЗапрос, ЭтоЗапросДокументации)
	...
	Запрос.Параметры.Вставить("Метод", Перечисления.ВидыМетодовHTTPЗапроса[HTTPЗапрос.HTTPМетод]);
	Запрос.Параметры.Вставить("ВариантЗапуска", Перечисления.СпособыВызоваДополнительныхОбработок.ВызовHTTPСервиса);
	Результат = Запрос.Выполнить(); 
